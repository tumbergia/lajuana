...
from app.common.enums import ParticipantFormStatus, PaymentStatus, ReservationStatus, UserRole
        ...
        *,
        ...
        if reservation.holder_phone:
            ...
            token_hash = hashlib.sha256(
                (raw_token + settings.participant_form_token_secret).encode("utf-8")
            ).hexdigest()

            reservation.participant_form_token_hash = token_hash
            reservation.participant_form_created_at = datetime.now(UTC)
            reservation.participant_form_expires_at = datetime.now(UTC) + timedelta(
                days=settings.participant_form_token_expiry_days
            )
            ...
            message = (
                ...
                f"Por favor comparte este formulario con los participantes de la experiencia "
                f"para que registren sus datos: {form_url}"
            )
