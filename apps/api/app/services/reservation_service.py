...
from app.common.enums import PaymentStatus, ReservationStatus, ScheduleStatus
...
class ReservationService:
    ...
    async def list(self) -> list[ReservationDocument]:
        return await ReservationDocument.find_all().to_list()

    async def get(self, reservation_id: str) -> ReservationDocument:
    ...
    async def update(
        self, reservation_id: str, payload: dict, actor_id: PydanticObjectId | None = None
    ) -> ReservationDocument:
    ...
    async def confirm_reservation(
        self, reservation_id: str, actor_id: PydanticObjectId | None = None
    ) -> ReservationDocument:
        ...
        if reservation.schedule_id is None:
            raise ApiError(
                status_code=422,
                code="reservation.schedule_required",
        ...
        if reservation.requested_date is not None:
