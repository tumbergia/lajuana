import datetime
from app.services import OpsService, ReservationService, ParticipantService, EquineService, ScheduleService, SaddleService
...
class EquineHandler:
    ...
    async def consultar(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not name:
            ...
            if not equines: return "No hay equinos registrados."
        ...
        if not equine: return f"No encontré al equino {name}."
    ...
    async def actualizar(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not name:
            equines = await self.equine_service.list()
            if not equines: return "No hay equinos registrados para actualizar."
...
class ReservationHandler:
    def __init__(self, reservation_service: ReservationService, participant_service: ParticipantService):
    ...
    async def consultar(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not codigo:
            ...
            if not reservas: return "No hay reservas registradas."
            info = ["Lista de reservas registradas:"]
            for r in reservas:
                info.append(f"- Código: {r.code} | Estado: {r.status.value} | Participantes: {r.participant_count}")
            return "\n".join(info)
    ...
    async def actualizar(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not codigo:
            from app.common.enums import UserRole
            reservas = await self.reservation_service.list(actor_role=UserRole.ADMIN)
            if not reservas: return "No hay reservas registradas para actualizar."
...
class SaddleHandler:
    ...
    async def actualizar(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not codigo:
            ...
            if not saddles: return "No hay sillas registradas para actualizar."
...
class ServiceLogHandler:
    ...
    async def crear(self, parametros_busqueda: dict, datos_cambio: dict) -> str:
        ...
        if not reservation_id:
            from app.common.enums import UserRole
            reservas = await self.reservation_service.list(actor_role=UserRole.ADMIN)
            info = ["Falta la reserva relacionada para la bitácora. Selecciona una de estas reservas:"]
            for r in reservas:
                info.append(f"- {r.code} | {r.status.value}")
            return "\n".join(info)
