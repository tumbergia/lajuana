...
async def test_provider_factory_does_not_fallback_to_env_when_db_disables_ai(
    ...
    config_service = SimpleNamespace(get_ai_configuration_document=AsyncMock(return_value=config_doc))

    monkeypatch.setattr("app.ai.providers.factory.ConfigService", lambda: config_service)
    monkeypatch.setattr("app.ai.providers.factory.GeminiProvider", lambda: (_ for _ in ()).throw(AssertionError("env fallback used")))
