...
KNOWN_EXCLUSIONS: dict[type, set[str]] = {
    ReservationDocument: {
        ...
        "completed_at",
        "expire_at",
        "participant_form_sent_at",
        "participant_form_sent_by",
        "participant_form_send_count",
        "participant_form_last_message_id",
        "confirmation_message_sent_at",
        "confirmation_message_sent_by",
        "availability_lock_key",
        "blocks_day",
    },
    ...
    UserDocument: {
        ...
        "refresh_token_hash",
        "last_login_at",
        # Preferences are managed through dedicated endpoints.
...
def _check_coverage(doc_cls: type, schema_cls: type, label: str) -> None:
    ...
    assert not unmapped, f"{label}: Document fields not in {schema_cls.__name__}: {unmapped}"
